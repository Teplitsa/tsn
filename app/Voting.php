...
class Voting extends Model
{
    ...
    public function getInfo()
    {
        ...
        if (!is_null(UserVoting::where('voting_id', $this->id)->where('role', RoleTypesInVoting::CHAIRMAN)->first())) {
            $data['predsed'] = UserVoting::where('voting_id', $this->id)->where(
                'role',
                RoleTypesInVoting::CHAIRMAN
            )->first()->user_id;
        }
        if (!is_null(UserVoting::where('voting_id', $this->id)->where('role', RoleTypesInVoting::SECRETARY)->first())) {
            $data['secretar'] = UserVoting::where('voting_id', $this->id)->where(
                'role',
                RoleTypesInVoting::SECRETARY
            )->first()->user_id;
        }
    ...
    public function getInfo2()
    {
        ...
        $data['predsed'] = UserVoting::where('voting_id', $this->id)->where('role', RoleTypesInVoting::CHAIRMAN)->first(
        )->user_id;
        $data['secretar'] = UserVoting::where('voting_id', $this->id)->where(
            'role',
            RoleTypesInVoting::SECRETARY
    ...
    public function isCounter($id)
    {
        return in_array(
            $id,
            UserVoting::where('voting_id', $this->id)->where('role', RoleTypesInVoting::COUNTER)->pluck('user_id')->all(
            )
        );
    }
    ...
    {
        $this->attributes['closed_at'] = is_object($value) ? $value : \Carbon\Carbon::createFromFormat(
            'd.m.Y H:i',
            $value
        );
    }
    ...
    public function setProtocolAtAttribute($value = null)
    {
        ...
        $this->attributes['protocol_at'] = is_object($value) ? $value : \Carbon\Carbon::createFromFormat(
            'd.m.Y H:i',
            $value
        );
    }
    ...
    {
        $this->attributes['public_at'] = is_object($value) ? $value : \Carbon\Carbon::createFromFormat(
            'd.m.Y H:i',
            $value
        );
    }
    ...
    {
        $this->attributes['opened_at'] = is_object($value) ? $value : \Carbon\Carbon::createFromFormat(
            'd.m.Y H:i',
            $value
        );
    }
    ...
    {
        $this->attributes['end_at'] = is_object($value) ? $value : \Carbon\Carbon::createFromFormat(
            'd.m.Y H:i',
            $value
        );
    }
    ...
    public function isFullForCurrentUser(RegisteredFlat $flat)
    {
        ...
        $user_vote_items = 0;
        $u = $this->vote_items->map(
            function ($item) use ($flat, $user_vote_items) {
    ...
    {
        if (Carbon::now()->diffInHours($this->end_at) >= $this->created_at->diffInHours($this->end_at)) {
            return 100;
        }
    ...
    {
        if (round(
                Carbon::now()->diffInHours($this->end_at) / $this->created_at->diffInHours($this->end_at) * 100
            ) < 100
        ) {
            return true;
        }
