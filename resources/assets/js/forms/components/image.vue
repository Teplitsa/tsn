    ...
    <div>
        <div :class="{'form-group': true, 'has-error': form.errors.has(name) }">
            ...
            <div class="col-sm-10 dropzone dz-clickable">
                ...
                <div v-if="form[name]">
                    <img :style="'width:100%; max-width: ' + width + '; display: block; margin-bottom: 10px;'"
                         :src="form[name]"/>

                    <button class="btn btn-outline btn-success" @click.prevent="selectImage()">Выбрать</button>
                    <button class="btn btn-outline btn-danger" @click.prevent="removeImage()">Удалить</button>

                </div>

                <input type="file" style="display: none" accept="image/*" @change="onFileChange">
